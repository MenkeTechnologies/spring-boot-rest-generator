...
pub fn render_entity(entity: &Entity) -> String {
    ...
    s.push_str("//! `SeaORM` Entity, generated by spring-boot-rest-generator (rust-loco target).\n\n");
    s.push_str("use sea_orm::entity::prelude::*;\n");
    s.push_str("use serde::{Deserialize, Serialize};\n\n");
    s.push_str("#[derive(Clone, Debug, PartialEq, DeriveEntityModel, Eq, Serialize, Deserialize)]\n");
...
pub fn render_entities_prelude(entities: &[Entity]) -> String {
    ...
    for e in entities {
        ...
        let pascal = pascal_case(&snake);
        s.push_str(&format!(
            "pub use super::{snake}::Entity as {pascal};\n"
        ));
    }
    s
}
...
pub fn write_loco_project(
    ...
    for (i, entity) in entities.iter().enumerate() {
        let snake = snake_table_for(&entity.table_name);
        fs::write(entities_dir.join(format!("{snake}.rs")), render_entity(entity))?;
    ...
    merge_mod_rs(&models_dir.join("mod.rs"), entities, /*include_entities=*/ true)?;
    ...
    fs::write(
        root.join("src/app_routes.rs"),
        render_app_routes(entities),
    )?;
